...
class ImageGridTwo(itemView: View): AdapterContract.ViewHolder(itemView) {
    ...
    override fun bindData(data: AdapterContract.ViewHolderData) {
        ...
        FrescoHelper.setImages(data.photos[0]?.urls?.thumb, itemView.left_image)
        FrescoHelper.setImages(data.photos[1]?.urls?.thumb, itemView.middle_image)
        FrescoHelper.setImages(data.photos[2]?.urls?.thumb, itemView.right_image)
